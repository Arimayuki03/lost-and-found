...
_CONTENT_FIELDS = {'category', 'name', 'description', 'found_time', 'location', 'contact', 'image_url'}
...
def create_found_item():
    try:
        ...
        if (not data or 'category' not in data or 'name' not in data or 'description' not in data
                or 'found_time' not in data or 'contact' not in data or 'location' not in data):
        ...
        try:
            found_time = parse_local_datetime(data['found_time'], 'found_time')
        except (TypeError, ValueError):
            return jsonify({"error": "found_time 时间格式无效"}), 400
        ...
        found_item = FoundItem(
            ...
            found_time=found_time, location=location, contact=data['contact'],
            image_url=image_url, user_id=current_user, is_completed=is_completed
        )
        db.session.add(found_item)
        db.session.commit()
        current_app.logger.info(f"用户 {current_user} 创建拾物信息成功")
        return jsonify({"message": "拾物信息创建成功"}), 201
...
def update_found_item(item_id):
    try:
        ...
        if 'found_time' in data:
            try:
                data['found_time'] = parse_local_datetime(data['found_time'], 'found_time')
            except (TypeError, ValueError):
                return jsonify({"error": "found_time 时间格式无效"}), 400
        ...
        # 查找拾物记录
        found_item = FoundItem.query.filter_by(id=item_id, user_id=current_user).first()
        if not found_item:
            current_app.logger.warning(f"拾物信息 ID {item_id} 未找到或无权限编辑")
            return jsonify({"error": "拾物信息未找到或无权限编辑"}), 404

        # 记录原始状态，用于判断是否是重新发布
        original_is_completed = found_item.is_completed
        original_is_under_review = found_item.is_under_review
        ...
        if 'found_time' in data:
            found_item.found_time = data['found_time']
        if 'location' in data:
            found_item.location = data['location']
        if 'contact' in data:
            found_item.contact = data['contact']
        if 'image_url' in data:
            found_item.image_url = data['image_url']
        if 'is_completed' in data and not original_is_under_review:
            found_item.is_completed = data['is_completed']
            current_app.logger.info(f"更新完成状态: 从 {original_is_completed} 到 {data['is_completed']}")

            # 如果是将状态从"已归还"改为"招领中"（重新发布），需要重新审核
            ...
            # 如果是标记为已归还，则不需要审核
                ...
                # 保持当前审核状态不变
                current_app.logger.info(f"物品 ID {item_id} 标记为已归还，保持审核状态不变")

        # 只要出现内容字段就视为内容变更，需要重新审核（未知垃圾字段不触发，仅状态
        # 更新也不触发：语义比"非仅状态更新"更精确，避免无改动物品被踢出公开列表）
        if content_changed:
            found_item.is_under_review = True  # 设置为需要审核
        ...
        # 记录更新后的状态
        current_app.logger.info(
            f"更新后状态: is_completed={found_item.is_completed}, is_under_review={found_item.is_under_review}")

        current_app.logger.info(f"用户 {current_user} 更新拾物信息 ID {item_id} 成功")
        return jsonify({"message": "拾物信息更新成功", "is_under_review": found_item.is_under_review}), 200
    except SQLAlchemyError as e:
        db.session.rollback()
        current_app.logger.error(f"数据库错误: {str(e)}")
        return jsonify({"error": "数据库错误"}), 500
    except Exception as e:
        current_app.logger.error(f"更新拾物信息时发生意外错误: {str(e)}")
        return jsonify({"error": "发生意外错误"}), 500
...
def delete_found_item(item_id):
    try:
        ...
        current_app.logger.info(f"用户 {current_user_id} 尝试删除拾物信息 ID {item_id}")

        # 查找拾物记录
        found_item = FoundItem.query.get(item_id)
        if not found_item:
            current_app.logger.warning(f"拾物信息 ID {item_id} 未找到")
            return jsonify({"error": "拾物信息未找到"}), 404

        # 记录拾物信息的所有者ID
        current_app.logger.info(f"拾物信息 ID {item_id} 的所有者ID为 {found_item.user_id}")

        # 验证用户权限
        if str(found_item.user_id) != str(current_user_id):
            current_app.logger.warning(
                f"用户 {current_user_id} 无权限删除拾物信息 ID {item_id}，所有者ID为 {found_item.user_id}")
        ...
        db.session.commit()
        current_app.logger.info(f"用户 {current_user_id} 删除拾物信息 ID {item_id} 成功")
        return jsonify({"message": "拾物信息删除成功"}), 200
    except SQLAlchemyError as e:
        db.session.rollback()
        current_app.logger.error(f"数据库错误: {str(e)}")
        return jsonify({"error": "数据库错误"}), 500
    except Exception as e:
        current_app.logger.error(f"删除拾物信息时发生意外错误: {str(e)}")
        import traceback
...
def get_my_found_items():
    try:
        ...
        # 查询当前用户发布的所有拾物记录，应用分页
        query = FoundItem.query.filter_by(user_id=current_user)
        # 排序白名单与 /admin/found-items 保持一致（字段均真实存在于 FoundItem）；
        ...
        paginated_result = paginate_query(
            ...
            sortable_fields=['id', 'name', 'category', 'location',
                             'is_completed', 'is_under_review', 'found_time',
                             'created_at', 'updated_at'],
        )

        if not paginated_result or not paginated_result['items']:
            return jsonify({"message": "未找到拾物信息", "items": []}), 200

        current_app.logger.info(f"用户 {current_user} 查询拾物信息成功")

        # 将记录转换为字典列表
        found_items_list = [item.to_dict() for item in paginated_result['items']]
        ...
        current_app.logger.error(f"查询拾物信息时发生意外错误: {str(e)}")
        return jsonify({"error": "发生意外错误"}), 500
...
def get_my_found_item_detail(item_id):
    try:
        ...
        # 查询指定ID的拾物记录（必须是当前用户发布的）
        found_item = FoundItem.query.filter_by(id=item_id, user_id=current_user).first()

        # 如果找不到记录，尝试获取公共接口的数据
        if not found_item:
            current_app.logger.info(
                f"用户 {current_user} 查询的拾物信息 ID {item_id} 不是该用户发布的，尝试获取公共数据")

            # 查询公共接口的数据（必须是已审核的）
            public_item = FoundItem.query.filter_by(id=item_id, is_under_review=False).first()

            if not public_item:
                current_app.logger.warning(f"拾物信息 ID {item_id} 未找到或正在审核中")
                return jsonify({"error": "拾物信息未找到或正在审核中"}), 404

            # 返回公共数据
            current_app.logger.info(f"用户 {current_user} 查询公共拾物详情 ID {item_id} 成功")
            return jsonify(public_item.to_dict()), 200

        # 返回用户自己的拾物详情（包括审核中的）
        current_app.logger.info(f"用户 {current_user} 查询自己的拾物详情 ID {item_id} 成功")
        ...
        # 其他异常处理
        current_app.logger.error(f"查询拾物详情时发生意外错误: {str(e)}")
        return jsonify({"error": "发生意外错误"}), 500
